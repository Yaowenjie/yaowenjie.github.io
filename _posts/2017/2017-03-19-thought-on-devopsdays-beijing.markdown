...
## 会议整体流程与内容
&emsp;&emsp;首先，活动的国际化程度还是很值得认可的，除了像Patrick、John Wills这样的国外演讲嘉宾出席之外，还是可以看到不少国外的参会者，并且还配备了专业的同声传译团队和设备。参与的企业也都是与DevOps强关联的技术企业，虽然在会议内外难免会有不少相关企业的软硬广，但是除了个别以自家工具为主题缺乏实质内容的演讲之外，总体感觉也不算过分。
...
&emsp;&emsp;在参加DevOpsDays之前，我阅读了伍斌伍道长 [实例化DevOps原则](http://insights.thoughtworkers.org/instantiate-the-principles-of-devops/) 一文，深以为然，如果你还对DevOps知之甚少，推荐你阅读一下这篇文章。里面提到了两个相对主流的DevOps框架/原则：CALMS 和 The Three Way，之后在这次活动中的多个演讲中，都看到这两大框架的影子。本文我将不会围绕着这两个框架展开，而是以我自己的思路在4个维度叙述，当然，不可避免地，你会发现所有这些还是逃脱不了这两个框架/原则。
...
&emsp;&emsp;这三点，第一次看到可能还不是特别容易弄明白。但是如果你一直工作在敏捷团队内，这三点便不难理解。**Overall Goal** 表明所有团队成员都有同一个目标 - 往往是保证某个软件/更改按质按量的准时交付。**T-shaped** 表示团队成员的技能应该呈现T字型，稍微形象点解释的话，就是你在专注某项技术或者某个内容的开发时（T字的那一横），也要关注团队整体技术或者产品整体业务（T字那一竖）。**Co-located** 是说团队成员的位置是随时变动的，这其实是期望团队成员多和团队内外不同的人交流，加深合作沟通的一种表现（就像在结对编程中，我们也经常switch pair）。这三个特点，其实可以实现资源效率到流动效率的转换[❷](#ref-2)，关注的点在于产品交付完成，而不只是开发关注开发结果，运维关注运维内容，两者互相不满互相指责（其实全功能团队里没有绝对的Dev和Ops之分）。
...
<center><img class="center" src="{{ site.url }}/images/2017/feedback-loop.png"  width="60%" alt="feedback-loop"></center>
...
&emsp;&emsp;当然，DevOpsDays上面还是接触到几个不是很主流的，相对“新”的概念。如Patrick和多个嘉宾都提到的ChatOps，它就是一种强调团队沟通协作的理念，通过协作缩短反馈环节。而刘俊提到的SRE(Site Reliability Engineering)其实最早是由Google提出的一套运维理念[❾](#ref-9)，另一个概念AIOps偏重智能算法，也可能是未来的一种趋势。由于在这两个概念上未做过多研究，在这里就不做太多叙述啦。

&emsp;&emsp;另外，本次大会中似乎没有提及（当然，也有可能是提及了但我错过了），一个比较不错也可能成为未来趋势的新概念：Serverless无服务器架构。它其实是依托类似AWS这样的云服务，尤其是在像Lambda这样的东西出来之后，它似乎让大家进一步忘记软件服务开发的过程中基础设施管理的成本了，这里我也不会赘述啦，感兴趣的读者依然可以阅读Martin Fowler的[Serverless](https://martinfowler.com/articles/serverless.html)一文。
